...
| [`kunchenguid/quota-axi`](https://github.com/kunchenguid/quota-axi) | `quota-axi-v0.1.29` (flake input + npm `quota-axi@0.1.29`) | **Reads local provider auth sources** — inspects Claude/Codex/Cursor/Copilot/Grok/Kimi quota windows from on-disk auth. Read-only: no routing, no provider mutation. | Pinned to an exact release; behind the cooldown/review gate. Documented as read-only, but it touches local credential material, which is why it is rated privileged rather than low-capability. |
